...
const app = express();
const server = http.createServer(app); //create the http server here with express app pass in
...
setInterval(() => {
  io.sockets.sockets.forEach((socket) => {
    ...
    jwt.verify(token, process.env.ACCESS_TOKEN_SECRET, (err) => {
      if (err) {
        console.log(
          `Token expired or invalid for socket ${socket.id}. Disconnecting...`
        );
        socket.emit("logout", {
...
io.on("connection", (socket) => {
  ...
  socket.on("message", (data) => {
    ...
    saveMessageEvent.emit("saveMessage", {
      receiverId,
      message,
      tempId,
